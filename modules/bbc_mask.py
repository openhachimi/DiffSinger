...
def fast_fast_bbc_mask(mel2ph, mask_length=3, min_segment_length=5, mask_prob=0.3):
    ...
    segment_starts, segment_lengths, segment_values = _compute_segment_info_parallel(
...
def _compute_segment_info_parallel(masked_mel2ph, segment_ids, max_segments, seq_len, device):
...
def _apply_masks_parallel(masked_mel2ph, segment_starts, segment_lengths, mask_decisions,
                          mask_length, seq_len, device):
    result = masked_mel2ph.clone()
