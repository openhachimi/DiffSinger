...
def alf_to_figure(attn_soft, attn_hard, ph_seq=None, title=None):
    """Visualize ALF (Alignment Learning Framework) alignment as a heatmap.

    Args:
        attn_soft (np.ndarray): [T_mel, T_text] soft attention weights
        attn_hard (np.ndarray): [T_mel, T_text] hard (binarized) attention weights
        ph_seq (list[str]): phoneme labels for the y-axis ticks
    ...
    T_mel, T_text = attn_soft.shape
    fig, axes = plt.subplots(1, 2, figsize=(max(8, T_mel // 10), max(4, T_text // 2)))
    for ax, data, label in zip(axes, [attn_soft, attn_hard], ['soft', 'hard']):
        # imshow expects [rows=y, cols=x]; plot transposed so x=time, y=phoneme
